...
def construct_html_digest
  ...
  digest_time = with_time_zone("Asia/Singapore") { Chronic.parse(CONFIG_YAML["production"]['digest_time']) }
  last_week_digest_time = digest_time - SEVEN_DAYS
  ...
  responses_to_prompt = Response.all(:created_at.gte => prompt_time)
  responses_over_week = Response.all(:created_at.gte => last_week_digest_time, :created_at.lte => prompt_time)

  return nil if responses_to_prompt.count == 0 && responses_over_week.count == 0

  body += "<h3>Summaries</h3>" if responses_to_prompt.count > 0

  responses_to_prompt.each do |res|
    response_html = "
  ...
  body += "<h3>Done List</h3>" if responses_over_week.count > 0

  responses_over_week.each do |res|
   response_html = "
    <p><strong>#{res.user.name}</strong>, (#{res.user.role}) - #{res.created_at.strftime("%a, %d/%m/%Y")}:</p>
    <p>#{res.content.gsub(/\n/, "<br/>")}</p>
    <hr/>"
    body += response_html
  end
